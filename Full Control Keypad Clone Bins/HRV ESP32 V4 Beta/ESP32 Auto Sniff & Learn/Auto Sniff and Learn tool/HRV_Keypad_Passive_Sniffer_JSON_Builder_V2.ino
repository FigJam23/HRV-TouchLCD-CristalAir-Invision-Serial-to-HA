...
static bool isLikelyFanCommandBytes(const std::vector<uint8_t>& rx) {
  if (rx.size() != 7) return false;
  if (rx[0] != 0x31 || rx[1] != 0x01) return false;

  // Known/observed fan command suffixes:
  // Original RV21:           31 01 4E/4F/50 pp 1E 84 F0
  // Alternate/heat-transfer: 31 01 5F/60    pp 1E 84 70
  // Alternate startup seen:  31 01 5E/5F/60 pp 1E 04 70
  // Older capture variant:   31 01 96/97/98 pp 18 84 70
  bool suffixKnown =
    (rx[4] == 0x1E && rx[5] == 0x84 && rx[6] == 0xF0) ||
    (rx[4] == 0x1E && rx[5] == 0x84 && rx[6] == 0x70) ||
    (rx[4] == 0x1E && rx[5] == 0x04 && rx[6] == 0x70) ||
    (rx[4] == 0x18 && rx[5] == 0x84 && rx[6] == 0x70);

  return suffixKnown && rx[3] <= 100;
}

static bool isLikelyReplyOrPollBytes(const std::vector<uint8_t>& rx) {
  if (rx.empty()) return true;

  // Common controller replies / polling/status frames. Show these in Recent Frames,
  // but do not let them win fan low/medium/high learning.
  if (rx[0] == 0x30) return true;
  if (rx[0] == 0x41) return true;
  if (rx[0] == 0x42) return true;
  if (rx[0] == 0x46) return true;

  return false;
}

static int fanCandidateScore(const String& payload, const String& action, uint16_t count) {
  std::vector<uint8_t> rx;
  if (!parseHexPayload(payload, rx)) return -1000;

  int score = 0;

  if (isLikelyFanCommandBytes(rx)) {
    score += 1000;

    // Prefer values that make sense for the action being learned.
    if (action == "fan_off") {
      if (rx[3] == 0) score += 300;
      else score -= 100;
    } else if (action == "fan_on") {
      if (rx[3] > 0) score += 250;
      if (rx[3] >= 10 && rx[3] <= 40) score += 50;
    } else if (action == "fan_low") {
      if (rx[3] > 0 && rx[3] <= 35) score += 250;
      if (rx[3] == 0) score -= 150;
    } else if (action == "fan_med") {
      if (rx[3] >= 20 && rx[3] <= 70) score += 250;
    } else if (action == "fan_high") {
      if (rx[3] >= 40) score += 250;
    }

    // Known keypad command code bytes get a small boost.
    if (rx[2] == 0x4E || rx[2] == 0x4F || rx[2] == 0x50 ||
        rx[2] == 0x5E || rx[2] == 0x5F || rx[2] == 0x60 ||
        rx[2] == 0x96 || rx[2] == 0x97 || rx[2] == 0x98) {
      score += 75;
    }
  } else if (isLikelyReplyOrPollBytes(rx)) {
    score -= 300;
  } else if (payloadLooksUseful(payload)) {
    score += 10;
  }

  // Count still matters, but not enough to let repeated poll frames beat a single fan command.
  score += min((int)count * 5, 100);
...
static bool decodeFanPercentFromPayload(const String& payload, int& pct, String& source) {
  ...
  // Useful fallback before learning is complete: detect known fan-command-like frames directly.
  if (isLikelyFanCommandBytes(rx)) {
    pct = (int)rx[3];

    if (rx[4] == 0x1E && rx[5] == 0x84 && rx[6] == 0xF0) {
      source = "rv21_fallback";
    } else if (rx[4] == 0x1E && rx[5] == 0x84 && rx[6] == 0x70) {
      source = "alt_1E8470_fallback";
    } else if (rx[4] == 0x1E && rx[5] == 0x04 && rx[6] == 0x70) {
      source = "alt_1E0470_fallback";
    } else if (rx[4] == 0x18 && rx[5] == 0x84 && rx[6] == 0x70) {
      source = "alt_188470_fallback";
    } else {
      source = "fan_template_fallback";
    }
    return true;
  }
...
static void serviceCaptureWindows() {
  ...
  if (learnActive && (int32_t)(now - learnEndMs) >= 0) {
    ...
    for (auto &r : learnedPayloads) {
      if (!payloadLooksUseful(r.payload)) continue;
      int score = fanCandidateScore(r.payload, currentLearnAction, r.count);
      if (score > bestScore) {
        best = r.payload;
        bestScore = score;
        bestCount = r.count;
    ...
    Serial.print("LEARN FINISHED: ");
    Serial.print(currentLearnAction);
    Serial.print(" best=");
    Serial.print(best);
    Serial.print(" count=");
...
static void appendPayloadArray(String& out, const std::vector<PayloadRecord>& list, bool includeCounts) {
  ...
  for (size_t i = 0; i < list.size(); i++) {
    ...
    if (includeCounts) {
      ...
      out += ",\"score\":" + String(fanCandidateScore(list[i].payload, lastCompletedAction.length() ? lastCompletedAction : currentLearnAction, list[i].count));
      out += "}";
    } else {
